...
fn get_free_pages(min_address: usize, max_addr: usize, self_task: mach_port_t) -> Result<Vec<(usize, usize)>, &'static str> {
    ...
    while current_address <= max_address {
        ...
        let kr = unsafe {
            mach_vm_region(self_task, &mut actual_address, &mut available_size,
                           vm_region::VM_REGION_BASIC_INFO_64, mem::transmute(&mut region_info), &mut count, &mut object_name)
        };
...
fn try_allocate_buffer(page_address: usize, page_size: usize, settings: &BufferAllocatorSettings, self_task: mach_port_t, result_addr: &mut usize) -> bool {
    let mut results: [usize; 4] = [0; 4];
    let buffer_pointers = get_buffer_pointers_in_page_range(page_address, page_size, settings.size as usize, settings.min_address, settings.max_address, &mut results);

    for addr in buffer_pointers {
        let mut allocated: mach_vm_address_t = *addr as mach_vm_address_t;
        let kr = unsafe {
            mach_vm_allocate(self_task, &mut allocated, settings.size as mach_vm_size_t, 0)
        };
...
fn get_buffer_pointers_in_page_range(page_address: usize, page_size: usize, buffer_size: usize, minimum_ptr: usize, maximum_ptr: usize, results: &mut [usize; 4]) -> &[usize] {
    let page_start = page_address;
    let page_end = page_address + page_size;
    let allocation_granularity = CACHED.allocation_granularity; 
    unsafe {
        get_possible_buffer_addresses(minimum_ptr, maximum_ptr, page_start, page_end, buffer_size, allocation_granularity as usize, results)
    }
}